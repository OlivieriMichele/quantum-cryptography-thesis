...
L'algoritmo di Grover, introdotto da Lov Grover nel 1996~\cite{grover1996fast}, rappresenta uno dei pilastri della computazione quantistica. A differenza dell'algoritmo di Shor, che offre un'accelerazione esponenziale rispetto alla controparte classica, Grover fornisce un'accelerazione quadratica nella risoluzione di problemi di ricerca non strutturata.
...
Il funzionamento dell'algoritmo si basa sulla manipolazione delle ampiezze di probabilità degli stati quantistici attraverso la ripetizione di un ciclo denominato \textit{iterazione di Grover}. Gli elementi chiave sono i seguenti.

\paragraph{Inizializzazione.}
Il sistema viene preparato in una sovrapposizione uniforme di tutti i possibili $N$ stati mediante l'applicazione della trasformata di Walsh-Hadamard $H^{\otimes n}$, garantendo che ogni stato $\ket{x}$ abbia la stessa ampiezza iniziale $1/\sqrt{N}$:
\begin{equation}
    \ket{\psi_0} = H^{\otimes n}\ket{0}^{\otimes n} = \frac{1}{\sqrt{N}} \sum_{x=0}^{N-1} \ket{x}.
\end{equation}

\paragraph{L'Oracolo $\mathcal{O}$.}
L'oracolo è una funzione unitaria in grado di riconoscere la soluzione del problema. Matematicamente, esso agisce invertendo la fase dell'ampiezza dello stato marcato $\ket{\omega}$, lasciando invariati tutti gli altri stati:
\begin{equation}
    \mathcal{O}\ket{x} = (-1)^{f(x)}\ket{x},
\end{equation}
dove $f(x) = 1$ se $x = \omega$ (stato soluzione) e $f(x) = 0$ altrimenti.

\paragraph{L'Operatore di Diffusione $\mathcal{D}$.}
L'operatore di diffusione, definito come
\begin{equation}
    \mathcal{D} = 2\ket{\psi_0}\bra{\psi_0} - I,
\end{equation}
esegue un'operazione nota come \textit{inversione rispetto alla media} (inversion about average). Il suo effetto è quello di amplificare l'ampiezza dello stato marcato dall'oracolo e di attenuare quella degli stati rimanenti, sfruttando i fenomeni di interferenza costruttiva e distruttiva.

\subsection*{Esempio Numerico: Ricerca su 2 Qubit ($N=4$)}
...
\paragraph{1. Inizializzazione.}
Applichiamo la trasformata di Walsh-Hadamard $H^{\otimes 2}$ allo stato $\ket{00}$, ottenendo la sovrapposizione uniforme:
\begin{equation}
    \ket{\psi_0} = \frac{1}{2}\bigl(\ket{00} + \ket{01} + \ket{10} + \ket{11}\bigr).
\end{equation}
Tutte le ampiezze iniziali sono uguali: $\alpha_i = \frac{1}{2} = 0.5$.

\paragraph{2. Applicazione dell'Oracolo.}
Supponiamo che la soluzione sia lo stato $\ket{11}$. L'oracolo $\mathcal{O}$ inverte il segno dell'ampiezza di tale stato, lasciando invariati gli altri:
\begin{equation}
    \ket{\psi_1} = \mathcal{O}\ket{\psi_0} = \frac{1}{2}\ket{00} + \frac{1}{2}\ket{01} + \frac{1}{2}\ket{10} - \frac{1}{2}\ket{11}.
\end{equation}
Il vettore delle ampiezze è ora $\{\, 0.5,\; 0.5,\; 0.5,\; -0.5 \,\}$.

\paragraph{3. Inversione rispetto alla Media.}
Applichiamo l'operatore di diffusione $\mathcal{D}$, che realizza l'inversione rispetto alla media delle ampiezze.
\begin{enumerate}[label=(\roman*)]
    \item Calcoliamo la media delle ampiezze correnti:
    \begin{equation}
        \langle \alpha \rangle = \frac{1}{4}\bigl(0.5 + 0.5 + 0.5 + (-0.5)\bigr) = \frac{1}{4} \cdot 1.0 = 0.25.
    \end{equation}
    \item Applichiamo la trasformazione $\alpha_i \mapsto 2\langle\alpha\rangle - \alpha_i$ a ciascuna ampiezza:
    \begin{itemize}
        \item Stati non soluzione ($\alpha_i = 0.5$): \quad $2(0.25) - 0.5 = 0.0$.
        \item Stato soluzione ($\alpha_i = -0.5$): \quad $2(0.25) - (-0.5) = 1.0$.
    \end{itemize}
\end{enumerate}

\paragraph{4. Risultato.}
Lo stato del sistema dopo una singola iterazione di Grover è:
\begin{equation}
    \ket{\psi_2} = \mathcal{D}\ket{\psi_1} = \ket{11}.
\end{equation}
...
L'algoritmo di Grover ammette un'elegante interpretazione geometrica. Lo spazio di Hilbert rilevante può essere ridotto a un sottospazio bidimensionale, definito dal vettore soluzione $\ket{\omega}$ e dal vettore
\begin{equation}
...
\begin{itemize}
    ...
    \item abbiamo infine introdotto l'algoritmo di Grover, che fornisce un'accelerazione quadratica per problemi di ricerca non strutturata, con implicazioni dirette sulla sicurezza dei sistemi a chiave simmetrica.
\end{itemize}
Ora che conosciamo il funzionamento matematico/teorico gli algoritmi di Shor e Grover, possiamo passare alla prossima fase della trattazione: analizzare la risposta della comunità crittografica a questa minaccia: la crittografia post-quantistica.