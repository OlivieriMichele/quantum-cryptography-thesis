...
\begin{abstract}	
La sicurezza digitale moderna pone le sue fondamenta sulla crittografia a chiave pubblica, che a sua volta delega la sua robustezza alla difficoltà computazionale di problemi matematici come il problema della fattorizzazione e del logaritmo discreto. 
Tuttavia, l'avvento dei computer quantistici minaccia di compromettere la sicurezza di questi sistemi, poiché algoritmi quantistici come quello di Shor possono risolvere tali problemi in tempo polinomiale sfruttando la trasformata di Fourier quantistica per individuare la periodicità di funzioni modulari.
Sebbene i computer quantistici attuali non siano ancora in grado di violare chiavi crittografiche reali, il modello di minaccia ``harvest now, decrypt later" rende la transizione urgente già oggi. 
...
Entriamo ora nel vivo della relazione: sviscerare i segreti dietro l'algoritmo di Shor per capire in che modo rompere il protocollo RSA appena descritto, e analogamente per l'algoritmo di Grover per capire che impatto ha sulla crittografia simmetrica.
Per farlo avremo prima bisogno di introdurre alcuni concetti di base della computazione quantistica che tali algoritmi utilizzano per ottenere i loro risultati. 
Una volta compresi questi concetti potremo procedere alla spiegazione degli algoritmi.
...
\label{chap:Crittografia post quantistica}
%----------------------------------------------------------------------------------------

La crittografia post-quantistica, anche nota come crittografia quantum-safe o quantum-resistant, è quell'ambito della ricerca e dello sviluppo di algoritmi crittografici (solitamente a chiave pubblica) progettati per essere sicuri contro attacchi crittanalitici effettuati da computer quantistici.
...
\begin{enumerate}
    ...
    La PQC si basa su problemi matematici che, allo stato attuale della ricerca, non presentano vulnerabilità esponenziali quantistiche. Nel prossimo capitolo: \hyperref[chap:Algoritmi post-quantistici]{``Algoritmi post-quantistici''} esamineremo nel dettaglio le principali famiglie di problemi matematici su cui si basano gli algoritmi post-quantistici.
...
\section{Ruolo del Nist e processo di standardizzazione}
...
\chapter{Algoritmi post-quantistici}
\label{chap:Algoritmi post-quantistici}
%----------------------------------------------------------------------------------------
...
\chapter{Implementazioni industriali}
...
\label{chap:Considerazioni finali}
%----------------------------------------------------------------------------------------

\section{Stato attuale dei computer quantistici}
...
\section{Stato attuale della PQC}

Il processo di standardizzazione del NIST, conclusosi nel 2024 con la 
pubblicazione di FIPS~203, 204 e 205~\cite{alagic2022nist}, ha segnato 
il passaggio dalla ricerca alla produzione. Come documentato nel 
Capitolo~\ref{chap:Implementazioni}, aziende come Apple, Google, Signal 
e Cloudflare hanno già distribuito protocolli ibridi su centinaia di 
milioni di dispositivi, dimostrando che la transizione non è solo 
teoricamente possibile ma industrialmente praticabile.

Le sfide che rimangono aperte riguardano principalmente i contesti con 
risorse limitate (IoT, smart card, sistemi embedded), dove l'overhead 
di chiavi e firme più grandi è ancora un vincolo critico, e la 
standardizzazione dei protocolli di autenticazione post-quantistica 
dell'identità, ancora affidata in larga parte a ECDSA anche nelle 
implementazioni più avanzate.

\section{Conclusioni}

La crittografia post-quantistica non è una risposta a una crisi già in atto:
è una risposta preventiva a una crisi prevedibile. Questa distinzione è
importante perché determina il tono con cui dovremmo guardare al lavoro
fatto finora.

Da un lato, c'è motivo di ottimismo: gli standard esistono, le implementazioni
funzionano, le grandi infrastrutture sono in transizione. Dall'altro, la
migrazione dell'intera infrastruttura crittografica globale -- dai certificati
TLS alle firme firmware, dai protocolli di messaggistica alle smartcard
bancarie -- è un'operazione di scala e complessità senza precedenti, che
richiederà anni di lavoro coordinato tra industria, governi e comunità
accademica.

L'ottica con cui guardare questa tesi non è tanto tecnica quanto metodologica: la
sicurezza di un sistema crittografico non è mai una garanzia permanente, ma
il risultato di un processo continuo di valutazione, aggiornamento e
adattamento. Il passaggio dalla crittografia classica a quella post-quantistica
è, in questo senso, non un'eccezione alla storia della crittografia -- ma
...
Ringrazio il mio relatore, il Prof. Luciano Margara, per la sua disponibilità e 
per avermi affiancato in questo percorso di tesi.

Un ringraziamento speciale va alla mia famiglia, per aver sempre creduto in me 
e per avermi supportato a 360 gradi durante tutto il percorso universitario.

Infine, ringrazio di cuore i miei amici e tutte le meravigliose persone che ho incontrato 
in questo percorso di vita: da chi mi ha accompagnato durante le lunghe sessioni di studio 
a chi ha reso più leggera la vita con momenti di svago e divertimento. 
Ognuno di voi ha lasciato un segno, contribuendo alla mia crescita accademica 
ma, ancor di più, a quella personale.
